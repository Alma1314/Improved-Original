...
### 任务面板
- 220px高×256px宽深色面板，居中显示
- 上1/5：金色"每日任务" + 灰色刷新倒计时
- 中3/5：3个槽位平分，每个槽位左3/4为可滚动任务内容（scissor裁剪），右1/4为纯文字锁定按钮
- 下1/5：手动刷新按钮 + 关闭按钮（无边框深色按钮，hover 时文字变金）
- 任务内容可独立滚动，鼠标悬停滚轮操作，右侧滚动条指示器
- 每槽位：编号、任务名、每目标独立进度条（蓝/绿）、奖励列表（黄/绿）
- 模糊背景位于最下层，所有内容在其上正常显示
